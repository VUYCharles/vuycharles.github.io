...
import { setLang, useLang, detectBrowserLang } from './lib/i18n.js'
...
function Shell() {
  ...
  // S.lang empty means "never explicitly chosen" — fall back to the browser's language, so
  // a first-time French/German/... visitor doesn't have to dig into Settings to be understood.
  // Once the user picks anything in Settings, S.lang is persisted and takes over forever.
        ...
        <ErrorBoundary>
          {!authed ? <Login /> : needsMobileOnboarding ? <MobileOnboarding /> : (
            <Routes>
              ...
              {/* Coach and Admin routes are stripped from the PWA-only build (no backend,
                  no accounts). Deep links land on Home via the catch-all below. */}
