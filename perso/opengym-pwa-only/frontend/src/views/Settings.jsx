...
import { t, LANGS, INSTR_LANGS, detectBrowserLang } from '../lib/i18n.js'
  ...
  return <div className="narrow">
    ...
    <Section title={t('General')} footer={t('Note: switching units only changes the label — logged numbers are not converted.')}>
      <SelectRow
        ...
        value={S.lang || detectBrowserLang()} onChange={v => update(s => { s.lang = v })}
...
function NotificationsCard({ S, update, toast }) {
  ...
  // Le build DEMO n'a pas de backend, donc pas de push serveur possible
  // (l'ancien PushCard tapait /api/push/public-key et renvoyait 504). Les
  // alertes du rest-timer, elles, marchent via l'API Notification native du
  // navigateur (store/useUI.js) — la permission est demandée juste-in-time
  // au premier timer déclenché, pas depuis Settings.
  if (DEMO) return <DemoNotificationsCard />
  return <PushCard S={S} update={update} toast={toast} />
}

function DemoNotificationsCard() {
  return (
    <Section
      title={t('Notifications')}
      footer={t('Scheduled workout-day reminders need the self-hosted version — a demo build has no server to send them.')}
    >
      <Row
        icon="bell" iconTint="var(--red)"
        title={t('Rest-timer alerts')}
        subtitle={t('Fire automatically when a rest timer ends — the browser asks for permission the first time.')}
      />
    </Section>
  )
}
