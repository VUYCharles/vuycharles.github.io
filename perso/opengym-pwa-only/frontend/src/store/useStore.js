...
// coach-device.js was removed with the PWA-only trim. These no-op stubs keep the MOBILE-only
// import graph resolvable — `if (MOBILE)` folds to false in DEMO so the stubs are never
// actually called, and Vite tree-shakes the dead branch away.
const loadCoachDevice = async () => null
const saveCoachDevice = async () => null
const coachDeviceSettings = () => null

const KEY = 'gym_state_v1'
export const DEF = {
  // lang: '' means "never explicitly chosen — use navigator.language". A previously saved
  // profile keeps whatever it had ('en', 'fr', …), and Settings persists the user's pick.
  // App.jsx and Settings.jsx read the effective language via `S.lang || detectBrowserLang()`.
  unit: 'kg', restSec: 90, restPauseSec: 15, sound: true, timerFlash: false, keepAwake: true, lang: '',
